  ...
  for (uint32_t i = 0; i < nof_grants; i++) {
    if (grants[i].needs_pdcch) {
      ...
      if (logger.info.enabled()) {
        ...
        logger.info("PDCCH_UL: cc=%d, rnti=0x%x, %s, tti_tx_dl=%d", cc_idx, grants[i].dci.rnti, str, tti_tx_dl);
      }
    }
  }
  return SRSRAN_SUCCESS;
  ...
  for (uint32_t i = 0; i < nof_grants; i++) {
    ...
    if (rnti) {
      ...
      if (LOG_THIS(rnti) and logger.info.enabled()) {
        ...
        logger.info("PDCCH_DL: cc=%d, rnti=0x%x, %s, tti_tx_dl=%d", cc_idx, grants[i].dci.rnti, str, tti_tx_dl);
      }
    }
  }
  return 0;
