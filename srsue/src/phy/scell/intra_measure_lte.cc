...
#else
  const auto detected_cells = srsue::phy_adapter::ue_get_detected_cells(serving_cell);

  for(const auto e : detected_cells) {
    if(cells_to_measure.count(e) == 0) {
       Log(info, "add detected cell %u", e);
       cells_to_measure.insert(e);
     }
  }
#endif
